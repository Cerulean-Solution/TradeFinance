...
#             raise Exception(f"Error during multi-hop RAG analysis: {str(e)}")
    def analyze(
        ...
        """
        Analyze using multi-hop agentic RAG (Mode 3)
        """
        ...
        # ---------------------------------------------------
        # ✅ BUILD COMPREHENSIVE QUERY (LOGGING ONLY)
        # ---------------------------------------------------
        merged_query = self._build_comprehensive_query(
            lc_details, presented_documents
        )

        request_for_logging = self._build_request_for_logging(merged_query)

        # ---------------------------------------------------
        # ✅ ACTUAL QUESTION SENT TO RAG
        # ---------------------------------------------------
        # IMPORTANT: keep this SHORT and CLEAN
        question = f"Analyze discrepancies for {self.lc_type} against presented documents."

        try:
            print("Running Multi-Hop Agentic RAG analysis (Mode 3)...", flush=True)

            answer, debug = self.rag_pipeline.ask(
                question=question,          # ✅ CLEAN QUESTION ONLY
                max_iters=8,
                thread_id=None,
                recursion_limit=40,
            )

            final_answer = answer or "No answer generated."

            return {
                "request": request_for_logging,   # ✅ full LC + docs for DB
                "response": final_answer,
                "analysis": final_answer,
                "tokens": self.rag_pipeline.token_usage,
                "debug": debug,
