...
class SwiftMT700Formatter:
    ...
    #     """Return default date YYMMDD (today + shift_days)"""
    ...
    #     if 'sequence_of_total' in extracted_data:
    #         fields['27'] = f":27:{extracted_data['sequence_of_total']}"
    #     else:
    #         fields['27'] = ":27:1/1"
        
    #     # Field 40A: Form of Documentary Credit
    #     if 'form_of_documentary_credit' in extracted_data:
    #         form = self.clean_text(extracted_data['form_of_documentary_credit'])
    #         fields['40A'] = f":40A:{form}"
    #     else:
    #         fields['40A'] = ":40A:IRREVOCABLE"
    ...
    #     if 'date_and_place_of_expiry' in extracted_data:
    #         # expiry = extracted_data['date_and_place_of_expiry']
    #         # # Extract date and place
    #         # parts = expiry.split(',')
    #         # if len(parts) >= 2:
    #         #     date_exp = self.format_date(parts[0].strip())
    #         #     place_exp = self.clean_text(parts[1].strip())[:29]  # Max 29 chars for place
    #         #     fields['31D'] = f":31D:{date_exp}{place_exp}"
    #         # else:
    #         #     date_exp = self.format_date(expiry)
    #         #     fields['31D'] = f":31D:{date_exp}"
    #         # Field 31D: Date and Place of Expiry
    ...
    #             # Format date safely; use default if missing or invalid
    #             date_exp = self.format_date(date_part) if date_part else get_default_date(90)
                
    #             # Clean place and truncate to max 29 chars
    ...
    #             fields['31D'] = f":31D:{get_default_date(90)}"
    ...
    #         if len(parts) == 2:
    #             fields['50'] = self.format_field_50(parts[0], parts[1])
    #         else:
    #             fields['50'] = self.format_field_50(applicant, "")
    ...
    #         if len(parts) == 2:
    #             fields['59'] = self.format_field_59(parts[0], parts[1])
    #         else:
    #             fields['59'] = self.format_field_59(beneficiary, "")
    ...
    #         tolerance = extracted_data['percentage_credit_amount_tolerance']
            
    #         # Ensure tolerance is a string to avoid TypeError
    #         tolerance_str = str(tolerance or "")
    ...
    #     if 'available_with_by' in extracted_data:
    #         available = self.clean_text(extracted_data['available_with_by'])
    #         fields['41A'] = f":41A:{available}"
        
    #     # Field 42C: Drafts At
    #     if 'drafts_at' in extracted_data:
    #         drafts = self.clean_text(extracted_data['drafts_at'])
    #         fields['42C'] = f":42C:{drafts}"
        
    #     # Field 42A: Drawee
    #     if 'drawee' in extracted_data:
    #         drawee = self.clean_text(extracted_data['drawee'])
    #         fields['42A'] = f":42A:{drawee}"
        
    #     # Field 43P: Partial Shipments
    #     if 'partial_shipments' in extracted_data:
    #         partial = self.clean_text(extracted_data['partial_shipments'])
    #         fields['43P'] = f":43P:{partial}"
        
    #     # Field 43T: Transshipment
    #     if 'transshipment' in extracted_data:
    #         trans = self.clean_text(extracted_data['transshipment'])
    #         fields['43T'] = f":43T:{trans}"
        
    #     # Field 44A: Loading on Board/Dispatch/Taking in Charge at/from
    #     if 'loading_on_board_dispatch_taking_in_charge' in extracted_data:
    #         loading = self.clean_text(extracted_data['loading_on_board_dispatch_taking_in_charge'])
    #         # Remove incoterms if present
    #         loading = re.sub(r'(CIF|FOB|CFR|EXW|FCA|CPT|CIP|DAP|DPU|DDP)[,\s]*', '', loading, flags=re.IGNORECASE)
    #         fields['44A'] = f":44A:{loading[:65]}"
        
    #     # Field 44B: For Transportation To
    #     if 'for_transportation_to' in extracted_data:
    #         transport = self.clean_text(extracted_data['for_transportation_to'])
    #         fields['44B'] = f":44B:{transport[:65]}"
        
    #     # Field 44C: Latest Date of Shipment
    #     if 'latest_date_of_shipment' in extracted_data:
    #         shipment_date = self.format_date(extracted_data['latest_date_of_shipment'])
    #         fields['44C'] = f":44C:{shipment_date}"
        
    #     # Field 45A: Description of Goods and/or Services
    #     if 'description_of_goods_and_services' in extracted_data:
    #         fields['45A'] = self.format_field_45A(extracted_data['description_of_goods_and_services'])
        
    #     # Field 46A: Documents Required
    #     if 'documents_required' in extracted_data:
    #         fields['46A'] = self.format_field_46A(extracted_data['documents_required'])
        
    #     # Field 47A: Additional Conditions
    #     if 'additional_conditions' in extracted_data:
    #         fields['47A'] = self.format_field_47A(extracted_data['additional_conditions'])
        
    #     # Field 71B: Charges
    #     if 'charges' in extracted_data:
    #         charges = self.clean_text(extracted_data['charges'])
    #         fields['71B'] = f":71B:{charges}"
        
    #     # Field 48: Period for Presentation
    #     if 'period_for_presentation' in extracted_data:
    #         period = extracted_data.get('period_for_presentation')
    #         period_str = str(period or "")
    #         match = re.search(r'(\d+)', period_str)
    #         if match:
    #             days = match.group(1)
    #             fields['48'] = f":48:{days} DAYS AFTER SHIPMENT DATE"
    #         else:
    #             fields['48'] = f":48:{self.clean_text(period_str)}"

        
    #     # Field 49: Confirmation Instructions
    #     if 'confirmation_instructions' in extracted_data:
    #         confirm = self.clean_text(extracted_data['confirmation_instructions'])
    #         if confirm and confirm != "WITHOUT":
    #             fields['49'] = f":49:{confirm}"
        
    #     # Field 53A: Reimbursing Bank
    #     if 'reimbursing_bank' in extracted_data:
    #         reimb_bank = self.clean_text(extracted_data['reimbursing_bank'])
    #         fields['53A'] = f":53A:{reimb_bank}"
        
    #     # Field 78: Instructions to Paying/Accepting/Negotiating Bank
    #     if 'instructions_to_paying_accepting_negotiating_bank' in extracted_data:
    #         fields['78'] = self.format_field_78(extracted_data['instructions_to_paying_accepting_negotiating_bank'])
    ...
    def format_mt700(self, extracted_data: Dict[str, str], 
        ...
        for tag, key, method, max_len in [
            ...
            # ('48', 'period_for_presentation', lambda x: f"{re.search(r'(\d+)', str(x) or '0').group(1)} DAYS AFTER SHIPMENT DATE" if re.search(r'(\d+)', str(x) or '') else self.clean_text(str(x)), None),
