...
#         self.executor = ThreadPoolExecutor(max_workers=4)
    ...
    #     """
    #     Extract and accumulate token usage from AzureChatOpenAI response.
    #     Works with LangChain response_metadata.
    #     """
    #     try:
    ...
    #         self.token_usage["prompt_tokens"] += prompt_t
    #         self.token_usage["completion_tokens"] += comp_t
    #         self.token_usage["total_tokens"] += (prompt_t + comp_t)
...
#         batch = [sq["query"] for sq in sub_questions[:3]]
#         return {
#             "sub_questions": sub_questions,
#             "current_query_batch": batch,
#             "sub_question": batch[0] if batch else question
#         }
...
- Enhanced synthesis with confidence scoring
- Comprehensive answer verification
- Direct PDF text extraction (no truncation, complete sentences)
- Simple evidence table with source attribution
- Inline tooltip support for source tracking
"""
...
class EnhancedAgentState(TypedDict, total=False):
    ...
    # Source tracking (FINAL VERSION)
    source_mapping: Dict[
        str, Dict[str, Any]
    ]  # Maps segment IDs to source info with complete text
    evidence_table: List[Dict[str, str]]  # Simple evidence table
...
def _extract_complete_text(text: str, max_sentences: int = 3) -> str:
    """Extract complete text without word discontinuation.

    Args:
        text: Full text content
        max_sentences: Maximum number of complete sentences to extract
    ...
    complete_text = " ".join(sentences[:max_sentences])
...
def _format_docs_with_complete_text(
    docs: List[Document],
) -> Tuple[str, Dict[str, Dict[str, Any]], List[Dict[str, str]]]:
    """Format documents with complete text extraction (no truncation).

    Returns:
        Tuple of (formatted_context, source_mapping, evidence_table)
    """
    lines = []
    source_mapping = {}
    evidence_table = []
    ...
    for i, d in enumerate(docs, start=1):
        ...
        full_content = d.page_content or ""
        complete_text = _extract_complete_text(full_content, max_sentences=3)

        # Store source information with complete text (NOT truncated)
        source_mapping[segment_id] = {
            "source": src,
            "page": page,
            "title": d.metadata.get("title", ""),
            "complete_text": complete_text,  # Complete sentences, no truncation
            "full_content": full_content,
            "content_preview": (
                full_content.splitlines()[0][:100] if full_content else ""
            ),
        }

        # Add to evidence table - use complete text
        ...
        evidence_table.append(
            {
                ...
                "source": filename,
                "page": str(page),
                "evidence_text": complete_text,
            }
        )

        header = f"[{segment_id}] ({src} p.{page})"
        body = full_content
        lines.append(f"{header}\n{body}\n")

    return "\n".join(lines), source_mapping, evidence_table
...
def _replace_segment_ids_with_sources(
    answer: str, source_mapping: Dict[str, Dict[str, Any]]
) -> str:
    """Replace segment IDs (S1, S2, etc.) with source document names and page numbers.

    Args:
        answer: The final answer text containing segment IDs like [S1], [S2], etc.
        source_mapping: Dictionary mapping segment IDs to source information

    Returns:
        Answer with segment IDs replaced by source document names and page numbers
    """
    ...
    # Replace all segment ID references with source information
    for segment_id, source_info in source_mapping.items():
        ...
        # Create replacement text with source document name and page number
        replacement = f"[{filename} p.{page}]"

        # Replace all occurrences of [S1], [S2], etc. with the source information
        result = result.replace(f"[{segment_id}]", replacement)
        result = result.replace(f"({segment_id})", replacement)
        result = result.replace(segment_id, replacement)

    return result


def _create_simple_evidence_table(evidence_table: List[Dict[str, str]]) -> str:
    """Create a simple markdown table from evidence data.

    Args:
        evidence_table: List of evidence dictionaries
    ...
    # Create table header
    table = "| Segment | Source Document | Page | Evidence Text |\n"
    table += "|---------|-----------------|------|----------------|\n"
    ...
    for row in evidence_table:
        ...
        # Escape pipes and clean up text
        evidence = evidence.replace("|", "\\|").replace("\n", " ")

        table += f"| {segment} | {source} | {page} | {evidence} |\n"

    return table


def _create_final_output(
    answer: str, evidence_table: List[Dict[str, str]], confidence: float
) -> str:
    """Create final output with answer and simple evidence table.

    Args:
        answer: The synthesized answer
        evidence_table: List of evidence entries
        confidence: Answer confidence score

    Returns:
        Formatted output with answer and table
    """
    output = []

    # Add confidence
    output.append(f"**Confidence Level: {confidence:.1%}**\n")
    ...
    output.append("## 📚 Key Supporting Evidence\n")

    # Add evidence table
    if evidence_table:
        output.append(_create_simple_evidence_table(evidence_table))
    else:
        output.append("No supporting evidence found.\n")

    return "".join(output)


# For HTML/Streamlit with tooltips
def _create_html_with_tooltips(
    answer: str, source_mapping: Dict[str, Dict[str, Any]]
) -> str:
    """Create HTML version with inline tooltips showing source information.

    Args:
        answer: Answer text with [Source p.Page] citations
        source_mapping: Source information mapping

    Returns:
        HTML with hover tooltips
    """
    html = '<div class="answer-with-tooltips">\n'
    ...
    for segment_id, source_info in source_mapping.items():
        ...
        page = source_info.get("page", "?")
        complete_text = source_info.get("complete_text", "")

        # Create tooltip
        tooltip_text = f"Source: {filename} (p.{page})\\nEvidence: {complete_text.replace(chr(34), chr(39))}"

        # Create HTML span with tooltip
        citation = f"[{filename} p.{page}]"
        html_citation = (
            f'<span class="citation" title="{tooltip_text}">{citation}</span>'
        )

        # Replace in result
        result = result.replace(citation, html_citation)
...
# For Streamlit display with expandable sections
def _create_streamlit_output(
    answer: str,
    evidence_table: List[Dict[str, str]],
    source_mapping: Dict[str, Dict[str, Any]],
    confidence: float,
) -> Dict[str, Any]:
    """Create structured output for Streamlit display.

    Returns:
        Dictionary with answer, table, and source info for Streamlit rendering
    """
    return {
        "answer": answer,
        ...
        "table_markdown": _create_simple_evidence_table(evidence_table),
    }
...
#         # ----------------------------------------------
    ...
    def _enhanced_plan(self, state: EnhancedAgentState) -> EnhancedAgentState:
        ...
        analysis = {
            "complexity": state.get("question_complexity", 5.0),
            "type": state.get("question_type", "analytical"),
            "estimated_hops": state.get("estimated_hops", 4),
            "required_evidence": state.get("required_evidence_types", []),
            "key_aspects": state.get("key_aspects", []),
        }

        context_hints = state.get("evidence_hints", [])
        ...
        response = chain.invoke(
            {
                ...
                "analysis": json.dumps(analysis),
                "context_hints": (
                    json.dumps(context_hints[-10:]) if context_hints else "[]"
                ),
                "gaps": json.dumps(gaps[-5:]) if gaps else "[]",
            }
        )

        planning = _safe_json_parse(
            response,
            {
                "sub_questions": [
                    {
        ...
        sub_questions = planning.get("sub_questions", [])
    ...
    def _parallel_retrieve(self, state: EnhancedAgentState) -> EnhancedAgentState:
        ...
        if not queries:
            return {
                ...
                "last_batch": [],
                "evidence_docs": state.get("evidence_docs", []),
                "evidence_hints": state.get("evidence_hints", []),
            }
        ...
        for query in queries:
            try:
                semantic_docs = self.semantic_retriever.invoke(query)
                similarity_docs = self.similarity_retriever.invoke(query)
                combined = _dedupe_docs(semantic_docs + similarity_docs)
                parallel_results[query] = combined
                all_docs.extend(combined)
            except Exception as e:
                print(f"Error retrieving for query '{query}': {e}")
                parallel_results[query] = []
    ...
    def _advanced_assess(self, state: EnhancedAgentState) -> EnhancedAgentState:
        ...
        context, source_mapping, evidence_table = _format_docs_with_complete_text(
            evidence_docs
        )
        ...
        # Update source mapping and evidence table
        current_mapping = state.get("source_mapping", {})
        current_mapping.update(source_mapping)
        current_table = evidence_table
    ...
    def _intelligent_decide(self, state: EnhancedAgentState) -> EnhancedAgentState:
        ...
        recent_success = (
            ...
            else "low" if last_batch_size == 0 else "medium"
        )
        ...
        decision = _safe_json_parse(
            response,
            {
                "decision": "continue" if iteration < max_iters else "stop",
                "confidence": 0.5,
                "reasoning": "Default decision",
                "stop_reasons": [],
                "continue_strategy": "expand_search",
                "estimated_remaining_hops": max(0, max_iters - iteration),
            },
        )

        should_stop = (
            decision.get("decision", "continue").lower() == "stop"
            or iteration >= max_iters
        )

        stop_reasons = decision.get("stop_reasons", [])
        if should_stop and not stop_reasons:
            stop_reasons = (
                ["Reached maximum iterations"]
                if iteration >= max_iters
                else ["Sufficient context gathered"]
            )
        ...
        return {
            ...
            "continue_probability": 1.0 - (0.5 if should_stop else 0.0),
            "stop_reasons": stop_reasons,
        }

    def _enhanced_synthesis(self, state: EnhancedAgentState) -> EnhancedAgentState:
        """Synthesize final answer from evidence documents with simple table."""
        question = state["question"]
        evidence_docs = state.get("evidence_docs", [])
        ...
        # Update source mapping and evidence table in state
        current_mapping = state.get("source_mapping", {})
        current_mapping.update(source_mapping)
        ...
        # Replace segment IDs with source document names and page numbers
        answer_with_sources = _replace_segment_ids_with_sources(
            answer.strip(), source_mapping
        )
        ...
        # Create final output with answer and simple evidence table
        final_output = _create_final_output(
            answer_with_sources, evidence_table, confidence
        )
    ...
    def _advanced_verify(self, state: EnhancedAgentState) -> EnhancedAgentState:
        ...
        context, _, _ = _format_docs_with_complete_text(evidence_docs)
