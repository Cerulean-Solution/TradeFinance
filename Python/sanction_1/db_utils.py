...
def save_tool_billing(
    ...
    try:
        print(f"[DB] Saving tool billing: {transaction_no}")
        conn = get_db_connection()
        cursor = conn.cursor()
        ...
        conn.commit()
        conn.close()
        log_message(f"Tool billing saved: {transaction_no}", "ACTIVITY")
        return True
    except Exception as e:
