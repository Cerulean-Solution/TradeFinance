...
const normalize = (val?: string | null) => (val ? val.toString().toLowerCase().trim() : '');
...
const SidebarMenu = () => {
  ...
  const filterMenuByPermissions = (items: TMenuConfig): TMenuConfig => {
    const currentUser = getCurrentUser();
    console.log('ghisabgaklst', currentUser);
    const roleName = normalize(currentUser);
    console.log('roleName', roleName);
    if (!roleName) {
      // No role → you can show nothing or full menu; I prefer nothing for security
      return [];
    }

    const hasViewPermissionForSection = (section: IMenuItemConfig): boolean => {
      const key = normalize(section.asset_key);
      if (!key) return true; // no RBAC rule → visible

      const match = roles.find(
        (r) => normalize(r.AssetName) === key && normalize(r.Role) === roleName
      );
      ...
      if (Array.isArray(perms)) {
        return perms.some((p) => normalize(p).includes('view'));
      }

      return normalize(perms).includes('view');
    };

    const filtered: TMenuConfig = items
      .map((section) => {
        // Only filter top-level RBAC groups by asset_key
        if (!hasViewPermissionForSection(section)) {
          return null;
        }

        // Optional: item-level permission filtering (using section.permission if needed)
        // For now we just keep children as is.
        if (section.children && section.children.length > 0) {
          return {
            ...section,
            children: section.children // could filter by item.permission later
          };
        }

        return section;
      })
      .filter((s): s is IMenuItemConfig => s !== null);

    return filtered;
  };

  const filteredMenu = filterMenuByPermissions(menuConfig);
