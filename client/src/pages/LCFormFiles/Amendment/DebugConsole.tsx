...
const DebugConsole: React.FC<DebugConsoleProps> = ({
    ...
    const saveLLMRequest = async () => {
      ...
      try {
        const response = await axios.post(
          'http://localhost:8000/api/lc/llm-request',
          {
  ...
  // useEffect(() => {
  //   const saveLLMResponse = async () => {
  //     const transactionId = localStorage.getItem('transaction_no');
  //     const requestId = localStorage.getItem('llm_request_id');

  //     if (!transactionId || !responseLog?.trim() || !requestId) return;

  //     try {
  //       const resJson = JSON.parse(responseLog || '{}');

  //       // Extract token usage for Extract, Verify, and Three-way amendments
  //       const extractUsage = resJson?.extracted?.usage ?? null;
  //       const verifyUsage = resJson?.verified?.usage ?? null;
  //       const threeWayUsage = resJson?.three_way?.usage ?? null;

  //       // Determine total tokens dynamically (whatever exists in response)
  //       const tokenCount =
  //         threeWayUsage?.total_tokens ??
  //         verifyUsage?.total_tokens ??
  //         extractUsage?.total_tokens ??
  //         resJson?.llm_metadata?.token_usage?.total_tokens ??
  //         resJson?.token_usage?.total_tokens ??
  //         null;

  //       const response = await axios.post(
  //         'http://localhost:8000/api/lc/llm-response',
  //         {
  //           transaction_no: transactionId,
  ...
  //           three_way_usage: threeWayUsage
  //         },
  //         { headers: { 'Content-Type': 'application/json' } }
  //       );

  //       console.log('LLM Response saved:', response.data);
  //     } catch (err: any) {
  //       console.error('Failed to save LLM response:', err.response || err);
  //     }
  //   };
          ...
          <CardTitle>
            ...
            {/* <div className="card min-w-full mt-4">
              <div className="card-table">
                <table className="table table-border align-middle text-gray-700 font-medium text-md w-full table-fixed">
                  <thead>
                    <tr>
                      <th className="w-1/2 text-left">Type</th>
                      <th className="w-1/2 text-left">Token Count</th>
                    </tr>
                  </thead>
                    ...
                    {(() => {
                      if (threeWayUsage) {
                        return (
                          <>
                            <tr>
                              <td className="w-1/2">Analysis (Request)</td>
                              <td className="w-1/2">{threeWayUsage.prompt_tokens}</td>
                            </tr>
                            <tr>
                              <td className="w-1/2">Analysis (Response)</td>
                              <td className="w-1/2">{threeWayUsage.completion_tokens}</td>
                            </tr>
                            <tr>
                              <td className="w-1/2">Analysis (Total)</td>
                              <td className="w-1/2">{threeWayUsage.total_tokens}</td>
                            </tr>
                          </>
                        );
                        ...
                        return (
                          <>
                            <tr>
                              <td className="w-1/2">Verify Amendment (Request)</td>
                              <td className="w-1/2">{verifyAmendmentUsage.prompt_tokens}</td>
                            </tr>
                            <tr>
                              <td className="w-1/2">Verify Amendment (Response)</td>
                              <td className="w-1/2">{verifyAmendmentUsage.completion_tokens}</td>
                            </tr>
                            <tr>
                              <td className="w-1/2">Verify Amendment (Total)</td>
                              <td className="w-1/2">{verifyAmendmentUsage.total_tokens}</td>
                            </tr>
                          </>
                        );
                        ...
                        return (
                          <>
                            <tr>
                              <td className="w-1/2">Extract Amendment (Request)</td>
                              <td className="w-1/2">{extracted.usage.prompt_tokens}</td>
                            </tr>
                            <tr>
                              <td className="w-1/2">Extract Amendment (Response)</td>
                              <td className="w-1/2">{extracted.usage.completion_tokens}</td>
                            </tr>
                            <tr>
                              <td className="w-1/2">Extract Amendment (Total)</td>
                              <td className="w-1/2">{extracted.usage.total_tokens}</td>
                            </tr>
                          </>
                        );
                      }
