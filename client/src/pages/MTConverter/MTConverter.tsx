...
export default function MTConverter() {
  ...
  const { data: instrumentsResponse, isLoading: instrumentsLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch('http://localhost:8000/api/lc/mt/instruments');
  ...
  useEffect(() => {
    const fetchReference = async () => {
      setLoadingRef(true);
      try {
        const res = await fetch("http://localhost:8000/api/lc/mt/mt-reference");
  ...
  useEffect(() => {
    ...
    const fetchMatrix = async () => {
      ...
      try {
        const res = await fetch(`http://localhost:8000/api/lc/mt/sample_matrix/${selectedInstrument}`);
 ...
 const loadSample = async () => {
  ...
  try {
    const response = await fetch('http://localhost:8000/api/lc/mt/sample/load', {
      method: 'POST',
  ...
  const handleReverseEngineer = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8000/api/lc/mt/reverse-engineer', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleConvertMT = async () => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:8000/api/lc/mt/convert-mt", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
...
`;
  ...
  const handleShare = () => {
    const subject = encodeURIComponent("Shared MT Conversion Result");
    const body = encodeURIComponent(`
MT TYPE: ${mtMessageType}
...
${JSON.stringify(extractedData, null, 2)}
  ...
  const handleInstrumentChange = async (instrumentCode: string) => {
    ...
    try {
      const matrixRes = await fetch(
        `http://localhost:8000/api/lc/mt/sample_matrix/${instrumentCode}`
      );
  ...
  return (
    <div className="card md:p-10 ">
      <h2 className="md:text-xl font-bold dark:text-white text-sm mb-5 uppercase">Check Availabilty & MT Converter</h2>
      ...
      <div className="card pb-2.5">
        ...
        <div className=" p-4 md:p-4 md:card-body grid gap-5">
          {/* ---------------------- Instrument ----------------------- */}
          <div className="w-full">
            <div className="flex items-baseline flex-wrap lg:flex-nowrap gap-2.5">
              <label className="form-label flex items-center gap-1 max-w-40 text-md">
        ...
        <div className="flex justify-between items-center mb-4 ">
          ...
          {selectedInstrument && selectedLifecycle && selectedVariation && (
            <button
              ...
              onClick={loadSample}
            >
              {sampleLoaded ? "Loaded Sample" : "Load Sample"}
            </button>
          )}
        </div>
        ...
        <div className="card pb-2.5 mt-5">
          <div className="card-body grid gap-5">
            ...
            <div className="flex justify-end gap-3 mb-3">
              ...
              <button className="btn btn-primary btn-outline px-4 py-2 text-xs md:text-md" onClick={handleShare}>Share</button>
      ...
      {/* <h3 className="card-title text-sm md:text-lg m-5">
        MT Message Type Reference
      </h3> */}

      {/* <div className="card min-w-full">
        {loadingRef ? (
          <p>Loading MT reference...</p>
        ) : error ? (
          <>
            <p className="text-red-600 font-semibold">{error}</p>
            <p className="text-gray-600">
              💡 Run <strong>INCREMENTAL_ADD_conv_Ins_mt.sql</strong> to enable this feature.
            </p>
          </>
        ) : mtReference.length === 0 ? (
          <p>No MT type mappings found.</p>
        ) : (
          <>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4">
              <div className="card text-center p-4 bg-gray-50">
                <b className="text-lg">{`Total Mappings`}</b>
                <span className="text-gray-500 font-semibold">{mtReference.length}</span>
              </div>
              <div className="card text-center p-4 bg-gray-50">
                <b className="text-lg">Unique Instruments</b>
                <span className="text-gray-500 font-semibold">
                  {new Set(mtReference.map(x => x.instrument_code)).size}
                </span>
              </div>

              <div className="card text-center p-4 bg-gray-50">
                <b className="text-lg">Unique MT Types</b>
                <span className="text-gray-500 font-semibold">
                  {new Set(mtReference.map(x => x.mt_message_type)).size}
                </span>
              </div>
            </div>
            ...
            <div className="card-table scrollable-x-auto">
              <table className="table align-middle text-gray-700 font-medium text-sm">
                <thead className="h-16">
                  <tr>
                    {Object.keys(mtReference[0]).map((colName) => (
